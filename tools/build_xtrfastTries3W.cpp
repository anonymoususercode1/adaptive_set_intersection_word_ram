#include <iostream>
#include <fstream>
#include <sdsl/bit_vectors.hpp>
#include "xTRFastBinaryTrie3W.hpp"

using namespace std;
using namespace sdsl;

bool verbose = false;
bool set_num_64 = false;
uint8_t part_rank_high = 4;

vector<uint64_t> read_inv_list_64(std::ifstream &input_stream, uint32_t n) {
    vector <uint64_t> il(n);
    input_stream.read(reinterpret_cast<char*>(il.data()), n * sizeof(uint64_t));
    return il;
}

vector<uint64_t> read_inv_list(std::ifstream &input_stream, uint32_t n) {

    vector <uint64_t> il;
    il.reserve(n);
    for (uint32_t i = 0; i < n; ++i) {
        uint32_t x;
        input_stream.read(reinterpret_cast<char *>(&x), 4);
        il.push_back((uint64_t)x);
    }
    return il;
}

template <typename wordType>
void buildCollection(std::string input_path, std::string out_path, std::string  out_data_path,
                     uint64_t min_size, uint64_t max_size, 
                     int rank_type, uint16_t wsize) {
    
    std::ifstream input_stream;
    input_stream.open(input_path, std::ios::binary | std::ios::in);
    if (!input_stream.is_open()) {
        cout << "Can't open input file:  " << input_path << endl;
        return;
    }
    std::ofstream out;
    if (out_path != ""){
        out.open(out_path, ios::out | ios::binary);
        if(!out.is_open()) {
            cout << "Can't open  output file:  " << out_path << endl;
            return;
        }
    }
    std::ofstream out_data;
    if (out_data_path != "") {
        out_data.open(out_data_path, std::ios::out);
        out_data << "set_number,elements_per_set" << std::endl; 
    }

    uint32_t _1;
    uint64_t u = 0;
    // Read universe of collection
    input_stream.read(reinterpret_cast<char *>(&_1), sizeof(_1));
    set_num_64 ? input_stream.read(reinterpret_cast<char *>(&u), sizeof(uint64_t)) : input_stream.read(reinterpret_cast<char *>(&u), sizeof(uint32_t));

    uint32_t nSets = 0;
    while (true) {
        uint32_t n;
        input_stream.read(reinterpret_cast<char *>(&n), 4);
        if (input_stream.eof()){
            break;
        }
        if (n > min_size && n <= max_size) {
            nSets++;
        }
        input_stream.seekg(n*(set_num_64 ? sizeof(uint64_t) : sizeof(uint32_t)), ios::cur);
    }
    // Set file pointer on the first set
    input_stream.clear();
    set_num_64 ? input_stream.seekg(sizeof(uint32_t) + sizeof(uint64_t), ios::beg) : input_stream.seekg(2*sizeof(uint32_t), ios::beg);
    // Write universe in out file
    if (out_path != "") {
        out.write(reinterpret_cast<char *> (&rank_type), sizeof(rank_type));
        out.write(reinterpret_cast<char *> (&wsize), sizeof(wsize));
        out.write(reinterpret_cast<char *> (&nSets), sizeof(nSets));
        out.write(reinterpret_cast<char *> (&_1), sizeof(_1));
        out.write(reinterpret_cast<char *> (&u), sizeof(u));
    }
    cout << "Universe: " << u << endl;

    uint64_t total_size = 0;
    uint64_t total_elements = 0;
    uint64_t n_il = 0;

    while (true) {
         uint32_t n;
        input_stream.read(reinterpret_cast<char *>(&n), 4);
        if (input_stream.eof()){
            break;
        }
        
        uint64_t trie_bytes_size = 0;
        if (n > min_size && n <= max_size) {
            vector <uint64_t> il = set_num_64 ?  read_inv_list_64(input_stream, n) : read_inv_list(input_stream, n);
            if (rank_type == 0) {
                
                xTRFastBinaryTrie3W<rank_support_v<1>, wordType> trie_v(il, u, part_rank_high);

                if (out_path != "") {
                    trie_v.serialize(out);
                }
                trie_bytes_size = trie_v.size_in_bytes();
                trie_v.free();
            }
            else {
                xTRFastBinaryTrie3W<rank_support_v5<1>, wordType> trie_v5(il, u, part_rank_high);

                if (out_path != "") {
                    trie_v5.serialize(out);
                }
                trie_bytes_size = trie_v5.size_in_bytes();
                trie_v5.free();
            }
            total_size += trie_bytes_size;
            total_elements += n;
            n_il++;

            if (out_data.is_open()) {
                out_data << n_il << "," << n << std::endl;
            }
            if ((n_il % 1000) == 0 && verbose) {
                cout << n_il  <<" Sets processed " << endl;
            }
        }
        else {
            input_stream.seekg(n*(set_num_64 ? sizeof(uint64_t) : sizeof(uint32_t)), ios::cur);
        }
        
    }
    input_stream.close();
    out.close();

    cout << "Total inverted lists: " << n_il << "| Bpi: " << (float)(total_size*8)/total_elements << endl;
    cout << "Total ints: " << total_elements << endl;
    cout << "----------------------------------------------------------\n";
    return;
}


int main(int argc, char** argv) {
    int mandatory = 3;

    if (argc < mandatory){
        std::cout   << "collection filename (*)"
                        "[--min_size m] "
                        "[--max_size m] "
                        "[--rank v] (*)"
                        "[--part_rank_high r] (*)"
                        "[--out output_filename]"
                        "[--out_data output_data_filename]"
                    <<
        std::endl;
        return 1;
    }

    int rank = 0;
    uint64_t min_size = 0;
    uint64_t max_size = -1;
    uint32_t block_size = 512;
    uint16_t wsize = 64;
    std::string output_filename = "";
    std::string output_data_filename = "";
    std::string input_filename = std::string(argv[1]);
    std::cout << input_filename << endl;
    
    for (int i = 2; i < argc; ++i){
        if (std::string(argv[i]) == "--min_size") {
            ++i;
            min_size = std::stoull(argv[i]);
        }
        if (std::string(argv[i]) == "--max_size") {
            ++i;
            max_size = std::stoull(argv[i]);
        }
        if (std::string(argv[i]) == "--rank") {
            ++i;
            if (std::string(argv[i]) == "v") {
                rank = 0;
            }
            else if (std::string(argv[i]) == "il") {
                rank = 1;
                i++;
                block_size = std::atoi(argv[i]);
            }
            else {
                rank = 2;
            }
        }
        if (std::string(argv[i]) == "--out") {
            ++i;
            output_filename = std::string(argv[i]);
        }
        if (std::string(argv[i]) == "--out_data") {
            ++i;
            output_data_filename = std::string(argv[i]);
        }
        if (std::string(argv[i]) == "--wsize"){
            ++i;
            wsize = std::atoi(argv[i]);
        }
        if (std::string(argv[i]) == "--verbose")
            verbose = true;
        if (std::string(argv[i]) == "--part_rank_high"){
            ++i;
            uint16_t temp = std::stoul(argv[i]);
            part_rank_high = static_cast<uint8_t>(temp);
        }
        if (std::string(argv[i]) == "--set_num_64")
            set_num_64 = true;
    }
    
    std::cout << "Min size: " << min_size << std::endl;
    std::cout << "Max size: ";
    (max_size == (uint64_t)(-1) ? (std::cout << "inf") : (std::cout << max_size));
    std::cout << endl;
    std::cout << "Type of trie\n";
    std::cout << "* Rank: ";
    if (rank  == 0){
        std::cout << "rank v" << std::endl;
    }
    else if (rank == 1) {
        std::cout << "rank il" << std::endl;
        std::cout << "* Block-size: " << block_size << std::endl;
    }
    else {
        std::cout << "rank v5" << std::endl;
    }
    std::cout << "  set_num_64: " << (set_num_64 == 1 ? "true" : "false") << std::endl;
    std::cout << "* word size: " << wsize << std::endl;
    std::cout << "Output file name: "<< output_filename << endl;
    

    if (wsize == 64)
        buildCollection<uint64_t>(input_filename, output_filename, output_data_filename,  min_size, max_size, rank, wsize);
    else if (wsize == 32)
        buildCollection<uint32_t>(input_filename, output_filename,  output_data_filename, min_size, max_size, rank, wsize);
    else if (wsize == 16)
        buildCollection<uint16_t>(input_filename, output_filename, output_data_filename,  min_size, max_size, rank, wsize);
    else if (wsize == 8)
        buildCollection<uint8_t>(input_filename, output_filename, output_data_filename,  min_size, max_size, rank, wsize);
    
    return 0;
}