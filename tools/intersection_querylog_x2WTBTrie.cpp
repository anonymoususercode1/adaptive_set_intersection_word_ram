...
#include "x2WTBTrieIntersection.hpp"
...
map<uint64_t, x2WTBinaryTrie<rankType, wordType>*> loadTries(ifstream &in, vector<vector<uint64_t>> &queries, uint64_t n){
    ...
    map<uint64_t, x2WTBinaryTrie<rankType, wordType>*> tries; 
    uint64_t nIl = 0;
    uint32_t np = 0;
    for(uint32_t i = 0; i < n; ++i) {
        x2WTBinaryTrie<rankType, wordType>* trie = new x2WTBinaryTrie<rankType, wordType>();
        trie -> load(in);
        if (i == setIndexes[np]){
            tries.insert({i, trie});
            np++;
...
void performIntersections( std::ifstream &in_sequences, std::string query_path,
    ...
    for (auto q: queries) {
        vector<x2WTBinaryTrie<rankType, wordType>*> QTries;
        for (auto i: q){
