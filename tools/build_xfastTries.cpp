...
void buildCollection(std::string input_path, std::string out_path, std::string  out_data_path,
    ...
    while (true) {
        ...
        if (n > min_size && n <= max_size) {
            ...
            if (rank_type == 0) {
                xFastBinaryTrie<rank_support_v<1>, wordType> trie_v(il, u);
                if (runs)
                    trie_v.encodeRuns();
                if (out_path != "") {
                ...
                xFastBinaryTrie<rank_support_v5<1>, wordType> trie_v5(il, u);
                if (runs)
                    trie_v5.encodeRuns();
                if (out_path != "") {
...
int main(int argc, char** argv) {
    ...
    std::cout << "Output data file name: "<< output_data_filename << endl;
