...
#include "xRFastTrieIntersection.hpp"
...
map<uint64_t, xRFastBinaryTrie<rankType, wordType>*> loadTries(ifstream &in, vector<vector<uint64_t>> &queries, u_int64_t n){
    ...
    map<uint64_t, xRFastBinaryTrie<rankType, wordType>*> tries; 
    uint64_t nIl = 0;
    uint32_t np = 0;
    for(uint32_t i = 0; i < n; ++i) {
        xRFastBinaryTrie<rankType, wordType>* trie = new xRFastBinaryTrie<rankType, wordType>();
        trie -> load(in);
        if (i == setIndexes[np]){
            tries.insert({i, trie});
            np++;
...
void performIntersections( std::ifstream &in_sequences, std::string query_path,
                           std::string out_path, bool runs_encoded, bool parallel) {
    uint16_t rep = 10;
    std::ofstream out;
    if (out_path != "") {
    ...
    for (auto q: queries) {
        vector<xRFastBinaryTrie<rankType, wordType>*> QTries;
        for (auto i: q){
...
int main(int argc, char const *argv[]) {
    ...
    if (rankT == 0){
        if (wsize == 64)
            performIntersections<sdsl::rank_support_v<1>, uint64_t>(in_sequences, querylog_filename, output_filename, runs, parallel);
        else if (wsize == 32)
            performIntersections<sdsl::rank_support_v<1>, uint32_t>(in_sequences, querylog_filename, output_filename, runs, parallel);
        else if (wsize == 16)
            performIntersections<sdsl::rank_support_v<1>, uint16_t>(in_sequences, querylog_filename, output_filename, runs, parallel);
        else
            performIntersections<sdsl::rank_support_v<1>, uint8_t>(in_sequences, querylog_filename, output_filename, runs, parallel);
    }
    else{
        if (wsize == 64)
            performIntersections<sdsl::rank_support_v5<1>, uint64_t>(in_sequences, querylog_filename, output_filename, runs, parallel);
        else if (wsize == 32)
            performIntersections<sdsl::rank_support_v5<1>, uint32_t>(in_sequences, querylog_filename, output_filename, runs, parallel);
        else if (wsize == 16)
            performIntersections<sdsl::rank_support_v5<1>, uint16_t>(in_sequences, querylog_filename, output_filename, runs, parallel);
        else
            performIntersections<sdsl::rank_support_v5<1>, uint8_t>(in_sequences, querylog_filename, output_filename, runs, parallel);
    }
