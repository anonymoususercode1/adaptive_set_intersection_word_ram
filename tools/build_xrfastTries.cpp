...
bool verbose = false;
uint8_t part_rank_high = 4;
bool set_num_64 = false;

vector<uint64_t> read_inv_list_64(std::ifstream &input_stream, uint32_t n) {
    vector <uint64_t> il(n);
    input_stream.read(reinterpret_cast<char*>(il.data()), n * sizeof(uint64_t));
    return il;
}
...
void buildCollection(std::string input_path, std::string out_path, std::string  out_data_path,
                     uint64_t min_size, uint64_t max_size, 
                     int rank_type, bool runs, uint16_t wsize) {
    ...
    if (out_data_path != "") {
        out_data.open(out_data_path, std::ios::out);
        out_data << "set_number,elements_per_set" << std::endl; 
    }
    ...
    while (true) {
        ...
        if (n > min_size && n <= max_size) {
            ...
            if (rank_type == 0) {
                xRFastBinaryTrie<rank_support_v<1>, wordType> trie_v(il, u, part_rank_high);
                if (runs)
                    trie_v.encodeRuns();
                if (out_path != "") {
                ...
                xRFastBinaryTrie<rank_support_v5<1>, wordType> trie_v5(il, u, part_rank_high);
                if (runs)
                    trie_v5.encodeRuns();
                if (out_path != "") {
...
int main(int argc, char** argv) {
    ...
    for (int i = 2; i < argc; ++i){
        ...
        if (std::string(argv[i]) == "--out_data") {
            ++i;
            output_data_filename = std::string(argv[i]);
        }
        ...
        if (std::string(argv[i]) == "--verbose")
            verbose = true;
        if (std::string(argv[i]) == "--part_rank_high"){
            ++i;
            uint16_t temp = std::stoul(argv[i]);
            part_rank_high = static_cast<uint8_t>(temp);
        }
    ...
    if (wsize == 64)
        buildCollection<uint64_t>(input_filename, output_filename, output_data_filename, min_size, max_size, rank, runs, wsize);
    else if (wsize == 32)
        buildCollection<uint32_t>(input_filename, output_filename, output_data_filename, min_size, max_size, rank, runs, wsize);
    else if (wsize == 16)
        buildCollection<uint16_t>(input_filename, output_filename, output_data_filename, min_size, max_size, rank, runs, wsize);
    else if (wsize == 8)
        buildCollection<uint8_t>(input_filename, output_filename, output_data_filename, min_size, max_size, rank, runs, wsize);
