...
bool AND(vector<xRFastBinaryTrie3W<rankType, wordType>*> &Ts, uint64_t nTries, uint64_t &maxLevel,
    ...
    if (currLevel == maxLevel-1) {
        ...
        wordType wordMedium[16]; 
        uint64_t pos_medium[16];
        uint64_t pos_aux = 0;

        wordType wLow;
        
        for (uint16_t i = 0; i < nTries; ++i) 
            w &= Ts[i] -> getWordHigh(roots[i]);
        ...
        while (w != 0){
            ...
            for (uint16_t j = 0; j < nTries; ++j) {
                ...
                uint64_t masked_number = (uint64_t)Ts[j] -> getWordHigh(roots[j]) & mask;
                //Count the number of bits set to 1
                uint64_t pos = __builtin_popcountll(masked_number) -1;
                wordMedium[j] = Ts[j] -> getWordMedium(roots[j], pos, pos_aux);
                pos_medium[j] = pos_aux;
            ...
            while (wM != 0){
                ...
                for (uint16_t j = 0; j < nTries; ++j) {
                    // Apply the mask to the number
                    uint64_t masked_number_m = (uint64_t)wordMedium[j] & maskM;
                    //Count the number of bits set to 1
                    uint32_t posM = __builtin_popcountll(masked_number_m) -1;
                    aaa = Ts[j] -> getWordLowByMediumPos(pos_medium[j], posM);
                    wLow &= aaa;
                }
                ...
                while (wLow != 0){
                    ...
                    r.push_back((z * word_bits * word_bits + zMedium * word_bits + zLow) + prefix);
...
template<typename rankType, typename wordType>
void partialAND(vector<xRFastBinaryTrie3W<rankType, wordType>*> &Ts, uint16_t n_tries, uint64_t max_level, uint64_t curr_level, 
    ...
    if (runs && node00 == 0b00) {
        ...
        uint64_t range = ((uint64_t)1 << ((max_level+(uint64_t)(log2(pow(sizeof(wordType)*8,2)))-1) - curr_level))- 1;
    ...
    if (result == 0b10) {
        ...
        rightResult = (rightResult | ((uint64_t)1 << ((max_level+(uint64_t)(log2(pow(sizeof(wordType)*8,2)))-1)- curr_level - 1)));
        for (uint64_t i = 0; i < n_tries; ++i) {
            if (runs && tempActiveTries[i])
...
template<typename rankType, typename wordType>
vector<uint64_t> Intersect(vector<xRFastBinaryTrie3W<rankType, wordType>*> &Ts, bool parallel){
    ...
    if (parallel && level_of_cut <= height-2){
        ...
        partialAND<rankType, wordType>(Ts, nTries, height, 0, level_of_cut, 
                    ...
                    tpartialSolutions, threadRoots, threadActiveTries, false);
        ...
        while ((nThreads - usedThreads > 1) && (i < totalThreads)) {
            partialAND<rankType, wordType>(Ts, nTries, height, level_of_cut, level_of_cut+1, threadRoots[i], 
                        activeTries, tpartialSolutions[i], intersection, 
                        nextPartialSolutions, nextRoots, nextActiveTries, false);
...
// Rank v
template
vector<uint64_t> Intersect<rank_support_v<1>, uint64_t>(vector<xRFastBinaryTrie3W<rank_support_v<1>, uint64_t>*> &Ts, bool parallel);
template
vector<uint64_t> Intersect<rank_support_v<1>, uint32_t>(vector<xRFastBinaryTrie3W<rank_support_v<1>, uint32_t>*> &Ts, bool parallel);
template
vector<uint64_t> Intersect<rank_support_v<1>, uint16_t>(vector<xRFastBinaryTrie3W<rank_support_v<1>, uint16_t>*> &Ts, bool parallel);
template
vector<uint64_t> Intersect<rank_support_v<1>, uint8_t>(vector<xRFastBinaryTrie3W<rank_support_v<1>, uint8_t>*> &Ts, bool parallel);
// Rank v5
template
vector<uint64_t> Intersect<rank_support_v5<1>, uint64_t>(vector<xRFastBinaryTrie3W<rank_support_v5<1>, uint64_t>*> &Ts, bool parallel);
template
vector<uint64_t> Intersect<rank_support_v5<1>, uint32_t>(vector<xRFastBinaryTrie3W<rank_support_v5<1>, uint32_t>*> &Ts, bool parallel);
template
vector<uint64_t> Intersect<rank_support_v5<1>, uint16_t>(vector<xRFastBinaryTrie3W<rank_support_v5<1>, uint16_t>*> &Ts, bool parallel);
template
vector<uint64_t> Intersect<rank_support_v5<1>, uint8_t>(vector<xRFastBinaryTrie3W<rank_support_v5<1>, uint8_t>*> &Ts, bool parallel);