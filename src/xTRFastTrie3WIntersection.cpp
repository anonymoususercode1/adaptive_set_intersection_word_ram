...
bool AND(vector<xTRFastBinaryTrie3W<rankType, wordType>*> &Ts, uint64_t nTries, uint64_t &maxLevel,
    ...
    if (currLevel == maxLevel-1) {
        ...
        while (w != 0){
            ...
            for (uint16_t j = 0; j < nTries; ++j) {
                ...
                //Count the number of bits set to 1
                uint64_t pos = __builtin_popcountll(masked_number) -1;
                wordMedium[j] = Ts[j] -> getTrieLevelPos(posTrie[j]) -> getWordMedium(roots[j], pos, pos_aux);
            ...
            while (wM != 0){
                ...
                for (uint16_t j = 0; j < nTries; ++j) {
                    ...
                    wLow &= Ts[j] -> getTrieLevelPos(posTrie[j]) -> getWordLowByMediumPos(pos_medium[j], posM);
                }
...
template<typename rankType, typename wordType>
void IntersectPartition(vector<xTRFastBinaryTrie3W<rankType, wordType>*> &Ts, uint64_t &nTries, uint64_t &height, vector<uint64_t> &groupTrieP, uint64_t groupTrieA[64][16], vector<uint64_t> &intersection){
...
// Rank v
template
vector<uint64_t> Intersect<rank_support_v<1>, uint64_t>(vector<xTRFastBinaryTrie3W<rank_support_v<1>, uint64_t>*> &Ts, bool parallel, bool balance);
template
vector<uint64_t> Intersect<rank_support_v<1>, uint32_t>(vector<xTRFastBinaryTrie3W<rank_support_v<1>, uint32_t>*> &Ts, bool parallel, bool balance);
template
vector<uint64_t> Intersect<rank_support_v<1>, uint16_t>(vector<xTRFastBinaryTrie3W<rank_support_v<1>, uint16_t>*> &Ts, bool parallel, bool balance);
template
vector<uint64_t> Intersect<rank_support_v<1>, uint8_t>(vector<xTRFastBinaryTrie3W<rank_support_v<1>, uint8_t>*> &Ts, bool parallel, bool balance);
// Rank v5
template
vector<uint64_t> Intersect<rank_support_v5<1>, uint64_t>(vector<xTRFastBinaryTrie3W<rank_support_v5<1>, uint64_t>*> &Ts, bool parallel, bool balance);
template
vector<uint64_t> Intersect<rank_support_v5<1>, uint32_t>(vector<xTRFastBinaryTrie3W<rank_support_v5<1>, uint32_t>*> &Ts, bool parallel, bool balance);
template
vector<uint64_t> Intersect<rank_support_v5<1>, uint16_t>(vector<xTRFastBinaryTrie3W<rank_support_v5<1>, uint16_t>*> &Ts, bool parallel, bool balance);
template
vector<uint64_t> Intersect<rank_support_v5<1>, uint8_t>(vector<xTRFastBinaryTrie3W<rank_support_v5<1>, uint8_t>*> &Ts, bool parallel, bool balance);