...
void verifyEncode(std::string input_path, uint32_t min_size, bool rank_type) {
    ...
    while (true) {
        ...
        if (n > min_size){
            ...
            if (rank_type == 0) {
                xTFastBinaryTrie_da<rank_support_v<1>, wordType> trie = xTFastBinaryTrie_da<rank_support_v<1>, wordType>(il, u);
                ...
                xTFastBinaryTrie_da<rank_support_v5<1>, wordType> trie(il, u);
