...
void verifyEncode(std::string input_path, uint32_t min_size, bool rank_type) {
    ...
    // n=5;
    // vector <uint64_t> il = {
    //     10,   // idx_h=0, idx_m=0, idx_l=10
    //     69,   // idx_h=0, idx_m=1, idx_l=5
    //     86, 325, 567, 572, 602, 604, 608, 616, 632, 635, 647, 651, 652, 656, 657,
    //     4032, // idx_h=0, idx_m=63, idx_l=0
    //     4161, // idx_h=1, idx_m=1, idx_l=1
    //     8294  // idx_h=2, idx_m=2, idx_l=2 (Este elemento está en un bloque High diferente)
    // };
    n = il.size();
            if (rank_type == 0) {
                xRFastBinaryTrie3W<rank_support_v<1>, wordType> trie = xRFastBinaryTrie3W<rank_support_v<1>, wordType>(il, u, part_rank_high);
                ...
                // cout << "rank v5\n";
                xRFastBinaryTrie3W<rank_support_v5<1>, wordType> trie(il, u, part_rank_high);
                trie_bytes_size = trie.size_in_bytes();
                vector<uint64_t> decoded;
                trie.decode(decoded);
                if (decoded != il) {
...
int main(int argc, char** argv) {
    ...
    if (wsize == 64)
        verifyEncode<uint64_t>(input_filename, min_size, rank);
    else if (wsize == 32)
        verifyEncode<uint32_t>(input_filename, min_size, rank);
    else if (wsize == 16)
        verifyEncode<uint16_t>(input_filename, min_size, rank);
    else
        verifyEncode<uint8_t> (input_filename, min_size, rank);
