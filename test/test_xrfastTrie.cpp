    ...
    while (true) {
        ...
        if (n > min_size){
            ...
            if (rank_type == 0) {
                ...
                xRFastBinaryTrie<rank_support_v<1>, wordType> trie = xRFastBinaryTrie<rank_support_v<1>, wordType>(il, u, part_rank_high);
                if (runs){
                    // cout << "encoding runs\n";
                    trie.encodeRuns();
                }
                ...
                // cout << "rank v5\n";
                xRFastBinaryTrie<rank_support_v5<1>, wordType> trie(il, u, part_rank_high);
                if (runs)
                    trie.encodeRuns();
                trie_bytes_size = trie.size_in_bytes();
                vector<uint64_t> decoded;
