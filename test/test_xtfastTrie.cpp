...
void verifyEncode(std::string input_path, uint32_t min_size, bool rank_type) {
    ...
    while (true) {
        ...
        uint64_t trie_bytes_size_all;
        if (n > min_size){
            vector <uint64_t> il = read_inv_list(input_stream, n);
            if (rank_type == 0) {

                xTFastBinaryTrie<rank_support_v<1>, wordType> trie = xTFastBinaryTrie<rank_support_v<1>, wordType>(il, u);
                trie_bytes_size = trie.size_in_bytes();
                trie_bytes_size_all = trie.size_all_in_bytes();
                ...
                // cout << "rank v5\n";
                xTFastBinaryTrie<rank_support_v5<1>, wordType> trie(il, u);
                trie_bytes_size = trie.size_in_bytes();
                trie_bytes_size_all = trie.size_all_in_bytes();
                vector<uint64_t> decoded;
                trie.decode(decoded);
            ...
            total_size_all += trie_bytes_size_all;
            total_elements += n;
            n_il++;
    ...
    cout << "Total inverted lists: " << n_il << "| Bpi full: " << (float)(total_size_all*8)/total_elements << endl;
