...
class xTFastBinaryTrie{
    private:
        wordType firstLevel = 0;
        uint16_t height_u = 0;        
        xFastBinaryTrie<rankType, wordType>** trieLevel = nullptr;
    ...
    public:
        xTFastBinaryTrie() = default;
        ~xTFastBinaryTrie() {
            free(); // Free dynamic memory
        }

        // Build the trie with height log2(max(set)-1)+1
        xTFastBinaryTrie(vector<uint64_t> set){
            uint64_t u = set.back();
            xTFastBinaryTrie(set, u);
        };

        xTFastBinaryTrie(vector<uint64_t>& set, uint64_t u) {
            xTFastBinaryTrie::height_u = floor(log2(u - 1)) + 1;
            xTFastBinaryTrie::firstLevel = 0;
            ...
            vector<xFastBinaryTrie<rankType, wordType>*> trieLevelAux;
            ...
            for (uint64_t value : set) {                
                ...
                if (xTFastBinaryTrie::firstLevel == 0)
                {
                    xTFastBinaryTrie::firstLevel |= ((wordType)1 << first);
                } else if (!(xTFastBinaryTrie::firstLevel & ((wordType)1 << first))) 
                {
                    xTFastBinaryTrie::firstLevel |= ((wordType)1 << first);
                    vector<uint64_t> tempSet = std::move(currentSet); 
                    xFastBinaryTrie<rankType, wordType>* trie = new xFastBinaryTrie<rankType, wordType>(tempSet, new_u);
            ...
            trieLevel  = new xFastBinaryTrie<rankType, wordType>*[trieLevelAux.size()];
        ...
        inline xFastBinaryTrie<rankType, wordType>** getTrieLevel() const {
            return trieLevel;
        }

        inline xFastBinaryTrie<rankType, wordType>* getTrieLevelPos(uint64_t &pos) const {
        ...
        void load(std::istream &in) {  
           ...
           trieLevel = new xFastBinaryTrie<rankType, wordType>*[trieLevelSize];
            for (uint16_t i = 0; i < trieLevelSize; ++i) {
                trieLevel[i] = new xFastBinaryTrie<rankType, wordType>();
                trieLevel[i]->load(in);
            }
        ...
        inline uint64_t size_in_bytes() {
            ...
            for (uint16_t i = 0; i < trieLevelSize; ++i) {
                trieLevel_size += xTFastBinaryTrie::trieLevel[i]->size_in_bytes() + sizeof(xTFastBinaryTrie::trieLevel[i]);
            }

            return trieLevel_size + sizeof(wordType) + sizeof(uint16_t) +                   
                   sizeof(xTFastBinaryTrie::trieLevel);
        }
        ...
        inline uint64_t size_all_in_bytes() {
            ...
            return trieLevel_size + sizeof(wordType) + sizeof(uint16_t) +                   
                   sizeof(xTFastBinaryTrie::trieLevel) +  trieLevelSize * sizeof(uint32_t);
        }

        inline void decode( vector<uint64_t> &decoded) {

            wordType number = xTFastBinaryTrie::firstLevel;
            ...
            while (number != 0) {
                ...
                uint64_t upper_bits = bits_pos << (xTFastBinaryTrie::height_u - bits_needed);
