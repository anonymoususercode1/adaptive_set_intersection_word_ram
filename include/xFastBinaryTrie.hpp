#ifndef XFAST_BINARY_TRIE
#define XFAST_BINARY_TRIE
...
class xFastBinaryTrie{
    ...
    public:
        xFastBinaryTrie() = default;
        ~xFastBinaryTrie() = default;

        xFastBinaryTrie(vector<uint64_t> &set, uint64_t u) {
            xFastBinaryTrie::runs_encoded = false;
            uint32_t n = set.size();
            xFastBinaryTrie::height_u = floor(log2(u - 1)) +  1;
            xFastBinaryTrie::height = height_u - log2(pow(sizeof(wordType)*8,2)) + 1; //only works for height > 6
            uint64_t max_nodes     = 2 * (pow(2, height) - 1);
            max_nodes = max_nodes > MAX_BIT ? MAX_BIT : max_nodes; // utils: MAX_BIT 100000000000
            xFastBinaryTrie::bTrie = bit_vector(max_nodes, 0);
            ...
            while (!q.empty()) {
                ...
                if (level == height-1){
                    ...
                    xFastBinaryTrie::lastLevelLow.push_back(wordLow);
                    xFastBinaryTrie::lastLevelHigh.push_back(wordH);
                ...
                if (level == xFastBinaryTrie::height) break;
            }

            xFastBinaryTrie::bTrie.resize(index);
            xFastBinaryTrie::b_rank = rankType(&bTrie);
        };

        // Build the trie with height log2(max(set)-1)+1
        xFastBinaryTrie(vector<uint64_t> set){
            uint64_t u = set.back();
            xFastBinaryTrie(set, u);
        };


        inline uint16_t getHeight(){
            return xFastBinaryTrie::height;
        };
        ...
        inline wordType getWordHigh(uint64_t &node_id){
            return lastLevelHigh[node_id - (xFastBinaryTrie::bTrie.size()/2)];
        };        
        
        inline wordType getWordHighPos(uint64_t &node_id){
            return node_id - (xFastBinaryTrie::bTrie.size()/2);
        };

        inline wordType getWordLow(uint64_t &node_id, uint64_t low_pos){
            return lastLevelLow[node_id - (xFastBinaryTrie::bTrie.size()/2)][low_pos];
        };

        inline std::vector<wordType> getLastLevelLow(uint64_t &node_id){
            return lastLevelLow[node_id - (xFastBinaryTrie::bTrie.size()/2)];
        };
        ...
        inline uint64_t getLeftChild(uint64_t &node_id, uint16_t level) {
            if (level >= getHeight() - 1)
                ...
                return xFastBinaryTrie::b_rank((2*node_id)+1);
        };
        ...
        inline uint64_t getRightChild(uint64_t &node_id, uint16_t level) {
             if (level >= getHeight() - 1)
                ...
                return xFastBinaryTrie::b_rank((2*node_id)+2);
        ...
        inline uint64_t size_in_bytes() {
            ...
            uint64_t bv_size = sdsl::size_in_bytes(xFastBinaryTrie::bTrie);
            uint64_t lastL_size = sizeof(xFastBinaryTrie::lastLevelHigh) + 
                                  sizeof(wordType)*xFastBinaryTrie::lastLevelHigh.size();
            uint64_t lastLLow_size = sizeof(vector<vector<wordType>>) + 
                                     xFastBinaryTrie::lastLevelLow.size() * sizeof(vector<wordType>);

            for (uint64_t i = 0; i < xFastBinaryTrie::lastLevelHigh.size(); i++)
            {                
                lastLLow_size+=sizeof(wordType)*popcount(xFastBinaryTrie::lastLevelHigh[i]);
            }

            uint64_t rank_size = sdsl::size_in_bytes(xFastBinaryTrie::b_rank);
            return bv_size +
                    rank_size +
                    lastL_size +
                    lastLLow_size +
        ...
        uint64_t serialize(std::ostream &out) {
            ...
            out.write(reinterpret_cast<char*>(&(xFastBinaryTrie::height_with_runs)), sizeof(xFastBinaryTrie::height_with_runs));
            out.write(reinterpret_cast<char*>(&(xFastBinaryTrie::empty_trie))      , sizeof(xFastBinaryTrie::empty_trie));
            out.write(reinterpret_cast<char*>(&(xFastBinaryTrie::runs_encoded))    , sizeof(xFastBinaryTrie::runs_encoded));
        ...
        void load(std::istream &in){
            ...
            in.read(reinterpret_cast<char*>(&(xFastBinaryTrie::height_with_runs)), sizeof(xFastBinaryTrie::height_with_runs));
            in.read(reinterpret_cast<char*>(&(xFastBinaryTrie::empty_trie))      , sizeof(xFastBinaryTrie::empty_trie));
            in.read(reinterpret_cast<char*>(&(xFastBinaryTrie::runs_encoded))    , sizeof(xFastBinaryTrie::runs_encoded));

            xFastBinaryTrie::bTrie.load(in);
            ...
            for (uint64_t i = 0; i < sizeLastLevelLow; ++i){
                ...
                xFastBinaryTrie::lastLevelLow.push_back(lowPart);
            }
        };
        ...
        void writeCompressTrie(vector<uint64_t> ones_to_write[], //vector array
            ...
            // End condition
            if (curr_level == (xFastBinaryTrie::height-1)) {
            ...
            if (node == 0b11) {
                ...
                if (its11) {
                    if (curr_level == xFastBinaryTrie::height - 2){
        ...
        void writeOnes(vector<uint64_t> ones_to_write[], uint64_t* level_pos){
            ...
            for (uint16_t level = 0; level < xFastBinaryTrie::height-1; ++level) {
            ...
            xFastBinaryTrie::height_with_runs = last_level + 1;
            xFastBinaryTrie::bTrie = bit_vector(bits_n, 0);
        ...
        inline void encodeRuns() {
            vector<uint64_t> ones_to_write[xFastBinaryTrie::height];
            uint64_t *level_pos = new uint64_t[xFastBinaryTrie::height];
            for(uint64_t i = 0; i < xFastBinaryTrie::height; ++i) level_pos[i] = 0;
            
            vector<wordType> newLastLevelHigh;
            vector<vector<wordType>> newLastLevelLow;
            bool itsOneOne = false;
            writeCompressTrie(ones_to_write, newLastLevelHigh, newLastLevelLow, level_pos, 0, 0, itsOneOne);
            xFastBinaryTrie::lastLevelHigh = newLastLevelHigh;
            xFastBinaryTrie::lastLevelLow = newLastLevelLow;
            
            writeOnes(ones_to_write, level_pos);
            xFastBinaryTrie::runs_encoded = true;
            delete[] level_pos;
        };
        ...
        inline void decode( vector<uint64_t> &decoded) {
            if (xFastBinaryTrie::runs_encoded) {
                if (xFastBinaryTrie::empty_trie) return;
                else {
