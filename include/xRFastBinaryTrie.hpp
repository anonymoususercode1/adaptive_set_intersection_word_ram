...
class xRFastBinaryTrie{
    ...
    public:
        xRFastBinaryTrie() = default;
        ~xRFastBinaryTrie() = default;

        xRFastBinaryTrie(vector<uint64_t> &set, uint64_t u, uint8_t n_part_rank) {
            xRFastBinaryTrie::runs_encoded = false;
            xRFastBinaryTrie::r_block = nullptr;
            xRFastBinaryTrie::n_part_rank = n_part_rank;
            uint32_t n = set.size();
            xRFastBinaryTrie::height_u = floor(log2(u - 1)) +  1;
            xRFastBinaryTrie::height = height_u - log2(pow(sizeof(wordType)*8,2)) + 1; //only works for height > 6
            uint64_t max_nodes     = 2 * (pow(2, height) - 1); 
            max_nodes = max_nodes > MAX_BIT ? MAX_BIT : max_nodes; // utils: MAX_BIT 100000000000
            xRFastBinaryTrie::bTrie = bit_vector(max_nodes, 0);
            ...
            while (!q.empty()) {
                ...
                if (level == height-1){
                    ...
                    for (uint64_t i = 0; i < word_bits; i++)
                    {
                        if (wordAux[i] != 0)
                        {
                            xRFastBinaryTrie::lastLevelLow.push_back(wordAux[i]);
                            wordH |= ((wordType)1 << i); 
                        }                        
                    }
                    xRFastBinaryTrie::lastLevelHigh.push_back(wordH);
                ...
                if (level == xRFastBinaryTrie::height) break;
            }

            buildRankLevelHigh();
            xRFastBinaryTrie::bTrie.resize(index);
            xRFastBinaryTrie::b_rank = rankType(&bTrie);
        };

        // Build the trie with height log2(max(set)-1)+1
        xRFastBinaryTrie(vector<uint64_t> set){
            uint8_t n_part_rank = 2;
            uint64_t u = set.back();
            xRFastBinaryTrie(set, u, n_part_rank);
        };


        inline uint16_t getHeight(){
            return xRFastBinaryTrie::height;
        };
        ...
        inline wordType getWordHigh(uint64_t &node_id){
            return lastLevelHigh[node_id - (xRFastBinaryTrie::bTrie.size()/2)];
        };

        inline wordType getWordHighPos(uint64_t &node_id){
            return node_id - (xRFastBinaryTrie::bTrie.size()/2);
        };

        inline wordType getWordLow(uint64_t &node_id, uint64_t high_bit_pos){
            uint64_t high_pos = node_id - (xRFastBinaryTrie::bTrie.size()/2);
        ...
        inline uint64_t getLeftChild(uint64_t &node_id, uint16_t level) {
            if (level >= getHeight() - 1)
                ...
                return xRFastBinaryTrie::b_rank((2*node_id)+1);
        };
        ...
        inline uint64_t getRightChild(uint64_t &node_id, uint16_t level) {
             if (level >= getHeight() - 1)
                ...
                return xRFastBinaryTrie::b_rank((2*node_id)+2);
        ...
        inline uint64_t size_in_bytes() {
            ...
            uint64_t bv_size = sdsl::size_in_bytes(xRFastBinaryTrie::bTrie);
            uint64_t lastLHigh_size = sizeof(xRFastBinaryTrie::lastLevelHigh) + sizeof(wordType)*xRFastBinaryTrie::lastLevelHigh.size();
            uint64_t rankLLHigh_size = sizeof(uint32_t*) + sizeof(uint32_t)*(xRFastBinaryTrie::lastLevelHigh.size()+(n_part_rank-1))/n_part_rank;
            uint64_t lastLLow_size = sizeof(xRFastBinaryTrie::lastLevelLow) + sizeof(wordType)*xRFastBinaryTrie::lastLevelLow.size();
            
            uint64_t rank_size = sdsl::size_in_bytes(xRFastBinaryTrie::b_rank);
            return bv_size +
                    rank_size +
                    lastLHigh_size +
                    rankLLHigh_size +
        ...
        uint64_t serialize(std::ostream &out) {
            ...
            out.write(reinterpret_cast<char*>(&(xRFastBinaryTrie::height_with_runs)), sizeof(xRFastBinaryTrie::height_with_runs));
            out.write(reinterpret_cast<char*>(&(xRFastBinaryTrie::empty_trie))      , sizeof(xRFastBinaryTrie::empty_trie));
            out.write(reinterpret_cast<char*>(&(xRFastBinaryTrie::runs_encoded))    , sizeof(xRFastBinaryTrie::runs_encoded));
            out.write(reinterpret_cast<char*>(&(xRFastBinaryTrie::n_part_rank))    , sizeof(xRFastBinaryTrie::n_part_rank));

            uint64_t bvs_size, rank_size;
            bvs_size  = bTrie.serialize(out);
            rank_size = b_rank.serialize(out);
            
            uint64_t size_last_level_high = xRFastBinaryTrie::lastLevelHigh.size();
            out.write(reinterpret_cast<char*>(&size_last_level_high), sizeof(uint64_t));
            for (wordType x: xRFastBinaryTrie::lastLevelHigh)
                out.write(reinterpret_cast<char*>(&x), sizeof(wordType));

            uint64_t size_last_level_low = xRFastBinaryTrie::lastLevelLow.size();
            out.write(reinterpret_cast<char*>(&size_last_level_low), sizeof(uint64_t));
            for (wordType x: xRFastBinaryTrie::lastLevelLow)
                out.write(reinterpret_cast<char*>(&x), sizeof(wordType));
            ...
            for (size_t i = 0; i < r_block_size; i++)
                out.write(reinterpret_cast<char*>(&xRFastBinaryTrie::r_block[i]), sizeof(uint32_t));
            

            return bvs_size + rank_size + 
                   sizeof(xRFastBinaryTrie::lastLevelHigh) + 
                   sizeof(xRFastBinaryTrie::lastLevelLow) + 
                   sizeof(xRFastBinaryTrie::r_block) + 
                   sizeof(n_part_rank) +
        ...
        void load(std::istream &in){
            ...
            in.read(reinterpret_cast<char*>(&(xRFastBinaryTrie::height_with_runs)), sizeof(xRFastBinaryTrie::height_with_runs));
            in.read(reinterpret_cast<char*>(&(xRFastBinaryTrie::empty_trie))      , sizeof(xRFastBinaryTrie::empty_trie));
            in.read(reinterpret_cast<char*>(&(xRFastBinaryTrie::runs_encoded))    , sizeof(xRFastBinaryTrie::runs_encoded));
            in.read(reinterpret_cast<char*>(&(xRFastBinaryTrie::n_part_rank))    , sizeof(xRFastBinaryTrie::n_part_rank));

            xRFastBinaryTrie::bTrie.load(in);
        ...
        void writeCompressTrie(vector<uint64_t> ones_to_write[], //vector array
            ...
            // End condition
            if (curr_level == (xRFastBinaryTrie::height-1)) {
            ...
            if (node == 0b11) {
                ...
                if (its11) {
                    if (curr_level == xRFastBinaryTrie::height - 2){
        ...
        void writeOnes(vector<uint64_t> ones_to_write[], uint64_t* level_pos){
            ...
            for (uint16_t level = 0; level < xRFastBinaryTrie::height-1; ++level) {
            ...
            xRFastBinaryTrie::height_with_runs = last_level + 1;
            xRFastBinaryTrie::bTrie = bit_vector(bits_n, 0);
        ...
        inline void encodeRuns() {
            vector<uint64_t> ones_to_write[xRFastBinaryTrie::height];
            uint64_t *level_pos = new uint64_t[xRFastBinaryTrie::height];
            for(uint64_t i = 0; i < xRFastBinaryTrie::height; ++i) level_pos[i] = 0;
            
            vector<wordType> newLastLevelHigh;
            vector<wordType> newLastLevelLow;
            bool itsOneOne = false;
            writeCompressTrie(ones_to_write, newLastLevelHigh, newLastLevelLow, level_pos, 0, 0, itsOneOne);
            xRFastBinaryTrie::lastLevelHigh = newLastLevelHigh;
            xRFastBinaryTrie::lastLevelLow = newLastLevelLow;
            buildRankLevelHigh();
            writeOnes(ones_to_write, level_pos);
            xRFastBinaryTrie::runs_encoded = true;
            delete[] level_pos;
        };
        ...
        inline void decode( vector<uint64_t> &decoded) {
            if (xRFastBinaryTrie::runs_encoded) {
                if (xRFastBinaryTrie::empty_trie) return;
                else {
        ...
        inline void buildRankLevelHigh() {
            ...
            if (xRFastBinaryTrie::r_block != nullptr) {
                delete[] xRFastBinaryTrie::r_block;
                xRFastBinaryTrie::r_block = nullptr;
            }
            uint32_t size_level = xRFastBinaryTrie::lastLevelHigh.size();
            xRFastBinaryTrie::r_block = new uint32_t[(size_level+(n_part_rank-1))/n_part_rank]();
            for (size_t i = 0; i < size_level; i++)
            {
                if (i%n_part_rank == 0)
                    xRFastBinaryTrie::r_block[curr_word++] = count;
                count += popcount(xRFastBinaryTrie::lastLevelHigh[i]);
            }
        }
        ...
        inline uint32_t rankLevelHigh(uint32_t pos) {
            ...
            uint32_t rank = xRFastBinaryTrie::r_block[r_pos];
            for (size_t i = start_pos; i < pos; i++)
            {   
                rank += popcount(xRFastBinaryTrie::lastLevelHigh[i]);
            }
